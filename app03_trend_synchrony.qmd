...
\end{split}
$$
1. Construct a sequence of $N$ statistics $\mathrm{WAVK}_{1}(k_{1}), \dots, \mathrm{WAVK}_{N}(k_{N})$.
Then, the synchrony test statistic is
$$
S_T = \sum_{i=1}^N k_{i}^{-1/2}\mathrm{WAVK}_{i}(k_{i}),
$$
...
$$S^*_T=\sum_{i=1}^N k^{-1/2}_{i} \mathrm{WAVK}^*_{i}(k_{i}).$$
