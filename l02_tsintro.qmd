...
This lecture aims to introduce Python data structures and core algorithmic techniques that form the foundation of efficient problem solving in interviews and confident in identifying the proper data structure for a problem, analyzing trade-offs, and implementing algorithms with optimal complexity.

**Objectives**

1. Define core data structures (lists, stacks, queues, hash tables, linked lists, trees, graphs) and identify when to use each.
1. Analyze patterns and pitfalls in data structures (e.g., collisions in hashing, stack overflow in recursion, complexity of list operations in Python).
1. Distinguish between common algorithmic paradigms (divide-and-conquer, greedy, dynamic programming, graph traversal).
1. Implement and evaluate algorithms for searching, sorting, traversal, and optimization problems.
1. Explain complexity analysis (time and space) and relate it to scalability and resource constraints.


**Reading materials**

* “Introduction to Algorithms” – Cormen, Leiserson, Rivest, Stein (CLRS), Ch. 1–5, 10–16 (fundamentals + data structures).
...
A dictionary in Python is a hash table–based data structure that stores key–value pairs.

- Keys must be immutable and unique 
- Values can be of any type.  
- Average-time complexity for insertion, lookup, and deletion is **O(1)**, but worst-case (with many collisions) can degrade to **O(n)**.  
...
```{python}
del scores["Bob"]
removed = scores.pop("Charlie", None)   # safer than del when unsure
scores, removed
```
...
scores = {"A":1, "B":2}
ks = scores.keys()
scores["C"] = 3               # views reflect changes
list(ks)                      # ['A','B','C'] (order preserved since 3.7+)
```

### `update`, merge (`|` / `|=` in Python 3.9+)
...
## Safe Accumulation: `get` / `setdefault`
...
|----------------------------------|--------------|------------|--------------------------------------|
| Insert / Update `d[k]=v`         | O(1)         | O(n)       | Resize or heavy collision can degrade |
| Lookup `d[k]` / `d.get(k)`       | O(1)         | O(n)       | Use `get` to avoid `KeyError`         |
| Delete `del d[k]` / `d.pop(k)`   | O(1)         | O(n)       | `pop(k, default)` is safer            |
| Membership `k in d`              | O(1)         | O(n)       | Hash + equality                       |
| Iterate keys/values/items        | O(n)         | O(n)       | Views are dynamic, reflect changes    |
